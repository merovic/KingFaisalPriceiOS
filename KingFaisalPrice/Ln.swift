...
class Ln {
    ...
    var introductionWord = MOLHLanguage.isRTLLanguage() ? "المقدمة" : "introduction"
    ...
    var angayza = MOLHLanguage.isRTLLanguage() ? "عن الجائزة" : "About KFP"
    
    var nabza = MOLHLanguage.isRTLLanguage() ? "نبذة مختصرة" : "Summary"
    
    var enshaa = MOLHLanguage.isRTLLanguage() ? "إنشاء الجائزة" : "Establishment"
    
    var ahdaf = MOLHLanguage.isRTLLanguage() ? "أهداف الجائزة" : "Objectives"
    
    var amana = MOLHLanguage.isRTLLanguage() ? "الأمانة العامة" : "General Secretariat"
    
    var tare5 = MOLHLanguage.isRTLLanguage() ? "تاريخ الجائزة" : "KFP History"
    
    var mokawnat = MOLHLanguage.isRTLLanguage() ? "مكونات الجائزة" : "Prize Components"
    
    var hay2a = MOLHLanguage.isRTLLanguage() ? "هيئة الجائزة" : "KFP Committee"
    ...
    var categories = MOLHLanguage.isRTLLanguage() ? "فروع الجائزة" : "Prize Categories"
    
    
    var eslam = MOLHLanguage.isRTLLanguage() ? "خدمة الإسلام" : "Service to Islam"
    
    var arabic = MOLHLanguage.isRTLLanguage() ? "اللغة العربية والأدب" : "Arabic Language and Literature"
    
    var studies = MOLHLanguage.isRTLLanguage() ? "الدراسات الإسلامية" : "Islamic Studies"
    
    var medical = MOLHLanguage.isRTLLanguage() ? "الطب" : "Medicine"
    ...
    var nomination = MOLHLanguage.isRTLLanguage() ? "الترشيح للجائزة" : "Nominations"
    ...
    var social = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    
    var face = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    var twitter = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    var insta = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    var youtube = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    var linkedin = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    var contact = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    var contactLink = MOLHLanguage.isRTLLanguage() ? "تسجيل جديد" : "Create New Account"
    
    
    var enter = MOLHLanguage.isRTLLanguage() ? "دخول" : "Enter"
    
    var title = MOLHLanguage.isRTLLanguage() ? "الرئيسية" : "Main"
    
    var search = MOLHLanguage.isRTLLanguage() ? "بحث اسم الفائز" : "Search Winner"

    var language = MOLHLanguage.isRTLLanguage() ? "EN" : "AR"
